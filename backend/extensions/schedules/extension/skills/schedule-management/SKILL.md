...
1. Call `schedules__list_schedules` when the target task is ambiguous.
...
- Scheduled runs are lightweight omni tasks; delegate to `artifacts__studio` only when the prompt requires documents or heavy writing.
